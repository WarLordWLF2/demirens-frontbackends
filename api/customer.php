...
class Demiren_customer
{
    ...
    function cancelReqAmenities($json)
    {
        ...
        $sql = "UPDATE tbl_booking_charges 
            SET booking_charge_status = 3, booking_return_datetime = NOW() 
            WHERE booking_charges_id = :bookingChargesId";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(":bookingChargesId", $data["bookingChargesId"]);
        $stmt->execute();
    ...
    function getBedPrice()
    {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        return $stmt->rowCount() > 0 ? $stmt->fetch(PDO::FETCH_ASSOC)["charges_master_price"] : 0;
    }
    ...
    function getExtraGuestAndBedPrice()
    {
        $extraGuestPrice = $this->getExtraGuestPrice();
        $bedPrice = $this->getBedPrice();
        return [
